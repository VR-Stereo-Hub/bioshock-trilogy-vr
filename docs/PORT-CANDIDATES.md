...
| **Every crosshair hidden during a scripted scene** | `bvr::vr::publish_scene_active(<your scene predicate>)` each frame, from the adapter | never published, so never suppressed | The core half (aim dot + aim laser) is game-agnostic and needs only a scene predicate to publish. The GAME'S OWN reticle is not covered by it - that goes through each game's engine SET handler, so BS2 and Infinite each need their own equivalent of BS1's `assert_crosshair`. Check first whether their scenes even draw a reticle; BS1's is a `ShockPlayer bReticleDisabled` flag and nothing about that transfers. |
