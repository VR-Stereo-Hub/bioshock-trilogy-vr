...
fullscreen) on an RTX 4060. The ImGui overlay initializes; the Insert-toggle keypress itself still
needs a human check. LAA flag confirmed YES (4 GB address space). The game's first launch generated
...
**User checklist (5 minutes, next time you play):**
1. Launch the game, press **Insert** — does the "BioShock VR 0.1.0" overlay appear/disappear?
2. Play a few minutes — any input weirdness, stutter, or crash with the mod installed?
3. Optional: run `.\tools\tail-log.ps1` in a terminal while playing (works live now).
