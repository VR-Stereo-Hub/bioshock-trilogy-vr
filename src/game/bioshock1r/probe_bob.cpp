...
// builds in session 63 each cost a play session - one to a repeating heap sweep,
// one to a gun that desynced between the eyes - and neither was something the
// user had asked to test.
//
// It WAS armed at boot for one build (2026-08-22, user directive: they were not
// going to open the console to type the command), and that build is what ended
// the hunt - bone 43 moved 2.91 UU in ABSOLUTE world space after our write while
// the pawn's own Z was flat, so the carrier was our own stale `baseLoc` and not
// the engine. The carrier is identified, so the suspension is over.
