// I4 vrrec: record + replay the per-frame head state. Design notes and the
// carried BS1 lessons are in recorder.h.
//
// Threading: every entry point here runs on the GAME thread (the drive tail
...
// they carry their own atomics. The pad/hand channels are recorded and
// re-fed for I7 forward-compatibility; on this game nothing consumes them
// yet (the xinput bridge is never enabled here in I4), so they are inert.
...
// Runaway backstop, not a target: ~3.7 h at 75 present-edges/s. Never trimmed
...
// The head quat and the driven camera are the head-replay evidence: without
// them a broken head replay would not show in the log. (BS1's version also
// prints the aim-R ray; Infinite has no aim chain until I7.)
void mark_line(const char* tag, uint32_t idx, const RecEntry& e, const FVector& loc,
               const FRotator& rot) {
    BVR_LOG("[rec] %s mark %u head=(%.4f %.4f %.4f | %.4f %.4f %.4f %.4f) "
            "cam=(%.1f %.1f %.1f | %d %d %d) pad=%04X %u %u %d %d %d %d",
            tag, idx, e.head.px, e.head.py, e.head.pz, e.head.qx, e.head.qy, e.head.qz,
            e.head.qw, loc.x, loc.y, loc.z, rot.pitch, rot.yaw, rot.roll, e.buttons, e.lt,
            e.rt, e.lx, e.ly, e.rx, e.ry);
}
...
bool read_file(const std::wstring& path) {
    // _wfsopen with _SH_DENYNO - see recorder.h.
...
void on_tick(const bvr::vr::HeadPose& consumedHead, bool vrDrove, bool liveHead,
    ...
    if (g_mode == Mode::Recording) {
        ...
        // The head for THIS frame was already served by replay_head at the
        // drive gate (cursor untouched there); now feed the same entry's
        // hands + pad so future consumers see one consistent frame, then
        // advance.
        ...
        if (g_cursor % 10 == 0) {
            ++g_marks;
            mark_line("PLAY", g_cursor, e, loc, rot);
        }
        ++g_cursor;
    }
}
...
void handle_command(const char* args) {
    char verb[16] = {};
    int consumed = 0;
    if (!args || sscanf_s(args, "%15s%n", verb, static_cast<unsigned>(sizeof verb),
                          &consumed) != 1) {
        BVR_LOG("[rec] usage: vrrec start|stop|play [file]|status");
        return;
    }
    // Raw line includes its trailing newline - see recorder.h.
    ...
    if (strcmp(verb, "start") == 0) {
        ...
        if (bvr::vr::session_live()) {
            BVR_LOG("[rec] play REFUSED: an XR session is live (two writers on the "
                    "funnel) - close the headset session first (bsivr off)");
            return;
        }
        std::wstring path;
        if (*rest) {
        ...
        const char* m = g_mode == Mode::Recording ? "RECORDING"
                        : g_mode == Mode::Playing ? "PLAYING"
                                                  : "idle";
        BVR_LOG("[rec] status: %s | entries=%u cursor=%u marks=%u | last file %s", m,
                static_cast<uint32_t>(g_entries.size()), g_cursor, g_marks,
                g_fileA[0] ? g_fileA : "(none)");
    } else {
        BVR_LOG("[rec] unknown 'vrrec %s' (start|stop|play [file]|status)", verb);
    }
}
